...
GE_ALIGN_HEAD(16) struct SIMD4F
{
    float x, y, z, w;
} GE_ALIGN_TAIL(16);
...
static inline void SIMD4F_USTORE4(SIMD4F& val, float *arr)
{
...
static inline void SIMD4F_USTORE2(SIMD4F& val, float *arr)
{
...
static inline SIMD4F SIMD4F_ZERO_W(const SIMD4F& s)
{
    return SIMD4F_CREATE(s.x, s.y, s.z, 0.0f);
}

static inline SIMD4F SIMD4F_ZERO_ZW(const SIMD4F& s)
{
    return SIMD4F_CREATE(s.x, s.y, 0.0f, 0.0f);
}

static inline SIMD4F SIMD4F_MERGE_HIGH(const SIMD4F& abcd, const SIMD4F& xyzw)
{
...
static inline SIMD4X4F SIMD4X4F_INVERSE(const SIMD4X4F& a)
{
    ...
    if (invdet.x == std::numeric_limits<float>::infinity())
    {
        ASSERT_MSG(false, "Error Matrix");
